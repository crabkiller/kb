...
- **微模块化方案**：`@web-widget` 是基于 Web Components 标准的微模块解决方案，核心解决 Monorepo 中的跨团队协作、跨框架混合运行（如 React 中运行 Vue）及渐进式架构迁移问题。
- **核心实现原理**：
  - **HTMLWebWidgetElement**：自定义元素作为容器，管理加载、启动、挂载等生命周期状态（LOADED, BOOTSTRAPPED, MOUNTED）。
...
- **syncCacheProvider 机制**：
  - **SSR 阶段**：执行异步请求并阻塞渲染，将结果存入 `lifecycleCache` 并序列化至 HTML 的 JSON 状态中。
