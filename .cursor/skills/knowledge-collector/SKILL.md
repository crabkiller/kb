...
1. **内容整理 (Summarize)**：
   ...
   - 将内容写入 Markdown 文件中，后续需要以文件（File）而非文本方式发送到采集器。
2. **生成标题 (Generate Title)**：根据内容生成一个简洁、具有概括性的标题。**标题必须遵循以下命名约束**：
   - **禁止非法字符**：严禁包含路径分隔符（`/`、`\`）和系统保留字符（`:`、`*`、`?`、`"`、`<`、`>`、`|`）。
   - **处理特殊符号**：若标题中包含技术术语（如 `ESA/ENS`），必须将 `/` 替换为 `-` 或空格，避免被识别为子目录。
   - **长度限制**：标题建议控制在 50 个字符以内。
   - **格式规范**：禁止以点（`.`）开头或结尾，禁止包含换行符。
   - **示例**：「Koa 中间件实现」 (正确)；「ESA/ENS 接入要点」 (错误，应为「ESA-ENS 接入要点」)。
